/**
 * Opening / hook settings — camada 3 C4 (contract §20.4).
 */

export type CompositionOpeningMode = "none" | "hook";

export type CompositionOpeningSettings = {
  /** Mode A = none (default cold open). Mode B = hook available. */
  mode?: CompositionOpeningMode;
  /** When false, hook range is preserved but not applied to export. */
  hookEnabled?: boolean;
  /** Hook range on the composed content timeline (seconds). */
  hookStartSec?: number;
  hookEndSec?: number;
};
...
export function hookDurationSec(
